package com.tongcheng.soothsay.other.freepool;
...
public class FishManager {
    ...
    private final int picLenght [] = new int []{6,8,12,6,6,6};           //有多少祯
    private final String picNames [] = new String []{"jinyu","diewei","wugui","hongwugui","hongyu","molongqing"};   //图片名字

    private final int JINYU_PIC = R.drawable.fsc_frame_jinyu;
    private final int WUGUI_PIC = R.drawable.fsc_frame_wugui;
    private final int WEIDIE_PIC = R.drawable.fsc_frame_diewei;
    ...
    public FishModel createFish(FishBean fishBean){
        ...
        AnimationDrawable ad = getFishAnim(fishBean);
        if(ad == null){
            return null;
        }

        fishModel.setAnim(ad);
        fishModel.setPath(getRandomPath());
        fishModel.setTag(fishBean);
        fishs.add(fishModel);
        return fishModel;
    }
    ...
    //将鱼的图片切割出来
    private AnimationDrawable getFishAnim(FishBean fishBean){
        String idName = fishBean.getPinyin();
        //祯动画的长度
        int length = 0 ;
        for (int i = 0; i < picNames.length; i++) {
            if (idName.equals(picNames[i])) {
                length = picLenght[i];
            }
        }
        try {
            int picId = context.getResources().getIdentifier("fsc_frame_" + idName,"drawable",context.getPackageName());
            Bitmap fishBit = BitmapFactory.decodeResource(context.getResources(),picId);
            fishWidth = fishBit.getWidth() / length;
