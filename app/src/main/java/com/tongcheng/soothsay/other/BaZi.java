...
/**
 *  getYearGanZhi(int hour);
 *  返回年月日时的天干地支
 * 根据西元的生日时辰获取某个人的生辰八字，以及农历生日 
 *@author Andy.wang  
 * */  
public class BaZi {  
    private int year;  //阳历年
    private int month;  //阳历月份
    private int day;  //阳历日期
   ...
   public final static String[] Zhi = {"子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"};  
    ...
    //返回指定月跟日的干支
    public String getMonthAndDayGanZhi(String month,String day){
        int monthI ;
        int dayI;

        if(month.length() >= 2){
            if(month.indexOf("0") == 0){
                monthI = Integer.valueOf(month.substring(1,month.length()));
            }else{
        ...
        if(day.length() >= 2){
            if(day.indexOf("0") == 0){
                dayI = Integer.valueOf(day.substring(1,day.length()));
            }else{
        ...
        return getMonthAndDayGanZhi(monthI,dayI);
    }

    //返回指定月跟日的干支
    public String getMonthAndDayGanZhi(int month,int day){
        int idx = (year - 1864) % 60;
        //没有过春节的话那么年还算上一年的，此处求的年份的干支
        String y = jiazhi[idx];
        ...
        if(idxm == 10) idxm = 0;
        //求的月份的干支
        m = Gan[(idxm + month - 2 ) % 10] + Zhi[(month) % 12];
    ...
    public String getYearGanZhi(int hour) {  
        ...
        */  
        ...
        offset=(offset % 5 )*2;  
        //求得时辰的干支     
        //改为23点为子时。edit by andy
     //    int hour1=hour;
        if(hour%2 ==0 ){
        	hour=hour/2;
        	  h=Gan[(offset+hour)%10]+Zhi[(hour)%12];
        }else if(hour%2 ==1 ){
        	hour=hour/2;
      	      h=Gan[(offset+hour+1)%10]+Zhi[(hour+1)%12];
      }	     
        //在此处输出我们的年月日时的天干地支  
        return y+","+m+","+d+","+h;  
    }  
...
/*    public static void main(String[] args){  
        ...
        try {  
                ...
                cal.setTime(sdf.parse("1988-04-08"));  
                 lunar = new (cal);
