...
import com.tongcheng.soothsay.adapter.calculation.BaziInputAdapter;
 ...
 * 八字输入用户信息
 */
public class BaziPaipanInputActivity extends BaseTitleActivity implements BaziPaipanInputConstant.View, TimePickerView.OnTimeSelectListener {

    @BindView(R.id.edit_ziwei_input)
    EditText editName;
    @BindView(R.id.rg_ziwei_input)
    RadioGroup rgSex;
    @BindView(R.id.rg_ziwei_input_man)
    RadioButton rbMan;
    @BindView(R.id.rg_ziwei_input_woman)
    RadioButton rbWoman;
    @BindView(R.id.tv_ziwei_input_date)
    TextView tvDate;
    @BindView(R.id.tv_ziwei_input_fenxi)
    TextView tvFenxi;
    @BindView(R.id.tv_ziwei_input_del)
    TextView tvDel;
    @BindView(R.id.lv_ziwei_input)
    ListView listView;
    @BindView(R.id.activity_ziwei_input)
    View layoutView;
    @BindView(R.id.til_ziwei_input)
    TextInputLayout til;

    private String sex = "男";
    private String name;
    private String time = "1900.08.08.08";
    private int icon = R.drawable.default_head_icon;

    private TimePickerView timeView;

    private BaziInputAdapter adapter;

    private BaziPaipanInputConstant.Presenter mPresenter;
    ...
    public void initView() {
        getBaseHeadView().showTitle(getResources().getString(R.string.title_bazi_input));
    }
    ...
    public void initListener() {
        ...
        rgSex.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                WindowUtil.closeInputMethod(BaziPaipanInputActivity.this);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                BaziUserBean bean = adapter.getDatas().get(position);
                time = bean.getDate();
                name = bean.getName();
                sex = bean.getSex();
    ...
    public void initData() {
        mPresenter = new BaziPaipanInputPresenter(this);
...
//            editName.setError("名字不能为空");
        ...
        GotoUtil.GoToActivityWithData(this, BaziMainActivity.class, map);
        return;
//            mPresenter.getPaipan(map);
    ...
    //显示历史记录
    @Override
    public void showHistory(List<BaziUserBean> beans) {
        if (adapter == null) {
            adapter = new BaziInputAdapter(this, beans, R.layout.item_ziwei_input);
            listView.setAdapter(adapter);

        } else if (beans == null) {
            adapter.notifyChangeData(new ArrayList<BaziUserBean>());
    ...
    @Override
    public void setPresenter(BaziPaipanInputConstant.Presenter presenter) {
